...
import art.arcane.iris.nativegen.v26_3_R1.NativeTransitionColumn;
import art.arcane.iris.nativegen.v26_3_R1.NativeTerrainHeightCache;
import art.arcane.iris.world.history.TerrainBoundarySignature;
import java.util.Optional;
import art.arcane.iris.nativegen.v26_3_R1.NativeGenerationWriteGuard;
...
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureGenerationException;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureStartInjector;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureReferenceEnvelope;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureLocateResults;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureLocatePersistence;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureOwnershipRecovery;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructurePostProcessor;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureReferenceRepair;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureSurfaceFitter;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureTerrainIntegrator;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureVegetationClearer;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureVerticalPlacer;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureVanillaLocator;
import art.arcane.iris.nativegen.v26_3_R1.NativeStructureVolumeIndex;
import art.arcane.iris.nativegen.v26_3_R1.WorldgenTerrainHeightmaps;
import art.arcane.iris.spi.IrisLogging;
import art.arcane.iris.spi.PlatformBlockState;
import art.arcane.iris.generation.block.IrisCustomData;
import art.arcane.iris.platform.reflect.WrappedField;
import art.arcane.iris.platform.reflect.WrappedReturningMethod;
...
public class IrisChunkGenerator extends CustomChunkGenerator implements LongPredicate {
    ...
    public CompletableFuture<ChunkAccess> createBiomes(RandomState randomstate, Blender blender, StructureManager structuremanager, ChunkAccess ichunkaccess) {
        ...
        try (BukkitChunkGenerator.GenerationStagePermit stage = requireGenerationStage("bukkit_nms_create_biomes");
            ...
            ichunkaccess.fillBiomesFromNoise(
                    (x, y, z) -> customBiomeSource.getVisibleNoiseBiomeWithActiveGenerationLease(
                            x, y, z, null, resolverState));
            return CompletableFuture.completedFuture(ichunkaccess);
        }
    }

    @Override
    public CompletableFuture<ChunkAccess> buildTerrain(ChunkAccess ichunkaccess, Blender blender, RandomState randomstate, StructureManager structuremanager, BiomeManager biomeManager, WorldGenRegion carverBiomeRegion, Set<Holder<Biome>> possibleBiomes) {
        ChunkPos chunkPos = ichunkaccess.getPos();
        ...
        try {
            ...
            try (GenerationHistoryRuntimeRouter.RuntimeRoute.RuntimeScope runtimeScope = openHistoryRuntimeScope(route);
                 IrisContext.Scope ignored = IrisContext.open(engine, lease.sessionId(), null)) {
                delegatePipeline = delegate.buildTerrain(
                        ichunkaccess, blender, randomstate, structuremanager, biomeManager, carverBiomeRegion, possibleBiomes);
            }
    ...
    @Override
    public WeightedList<MobSpawnSettings.SpawnerData> getMobsAt(Level level, StructureManager structuremanager, MobCategory enumcreaturetype, BlockPos blockposition) {
    ...
    private WeightedList<MobSpawnSettings.SpawnerData> getMobsAtWithActiveRuntime(
        ...
        if (vanillaSpawnBiome == null) {
            return delegate.getMobsAt(runtimeLevel, structuremanager, enumcreaturetype, blockposition);
        }

        WeightedList<MobSpawnSettings.SpawnerData> vanillaSpawns = biomeSpawns(vanillaSpawnBiome, enumcreaturetype);
        WeightedList<MobSpawnSettings.SpawnerData> resolvedSpawns = delegate.getMobsAt(
                runtimeLevel, structuremanager, enumcreaturetype, blockposition);
        WeightedList<MobSpawnSettings.SpawnerData> environmentSpawns = runtimeLevel.environmentAttributes()
                .getValue(EnvironmentAttributes.NATURAL_MOB_SPAWNS, blockposition).getMobsToSpawn(enumcreaturetype);
        if (resolvedSpawns != environmentSpawns) {
            return resolvedSpawns;
        }

        WeightedList<MobSpawnSettings.SpawnerData> explicitSpawns = biomeSpawns(holder, enumcreaturetype);
    ...
    @Override
    public void addDebugScreenInfo(List<String> list, RandomState randomstate, BlockPos blockposition, SamplerContext samplerContext) {
        delegate.addDebugScreenInfo(list, randomstate, blockposition, samplerContext);
    }
