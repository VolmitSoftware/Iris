 ...
 * <p>
 * Everything here is static and null-tolerant: a null or non-Iris {@link NativeWorld} produces false, null, or a
 * no-op rather than an exception, so a caller never has to pre-check whether a level is generated by Iris.
 * <p>
 * <b>Threading.</b> {@link #isIrisLevel(NativeWorld)}, {@link #isStudioLevel(NativeWorld)} and
 * {@link #getEngine(NativeWorld)} read the level's chunk generator reference and are safe from any thread once
 * the level is loaded. The mantle accessors are safe off the server thread but touch engine storage - see their
 * own notes. {@link #pregenerate(NativeWorld, int)} and {@link #registerProvider(ModdedDataProvider)} mutate
...
public final class IrisModdedAPI {
     ...
     */
    public static boolean isIrisLevel(NativeWorld level) {
        if (level == null) {
            return false;
        }
        return NativeWorldGenerators.find(level, IrisModdedChunkGenerator.class) != null;
    }
     ...
     */
    public static boolean isStudioLevel(NativeWorld level) {
     ...
     */
    public static Engine getEngine(NativeWorld level) {
        if (level == null) {
            return null;
        }
     ...
     */
    public static boolean pregenerate(NativeWorld level, int radiusBlocks) {
     ...
     */
    public static boolean pregenerate(NativeWorld level, int radiusBlocks, int centerBlockX, int centerBlockZ, boolean sync, boolean cached) {
        Engine engine = getEngine(level);
        if (engine == null) {
            return false;
        }
        return ModdedPregenJob.start(level, engine, new ModdedPregenJob.PregenOptions(
                radiusBlocks, centerBlockX, centerBlockZ, false, sync, cached));
    }
     ...
     */
    public static <T> T getMantleData(NativeWorld level, int x, int y, int z, Class<T> type) {
        Engine engine = getEngine(level);
        if (engine == null) {
            return null;
        }
     ...
     * <p>
     * Unlike {@link #getMantleData(NativeWorld, int, int, int, Class)}, a write creates the mantle region if it
     ...
     * remove values with {@link #deleteMantleData(NativeWorld, int, int, int, Class)}.
     * <p>
     ...
     * A closing or closed engine refuses the operation quietly (null / no-op) instead of racing its shutdown.
     */
    public static <T> void setMantleData(NativeWorld level, int x, int y, int z, T data) {
        Engine engine = getEngine(level);
     ...
     * A closing or closed engine refuses the operation quietly (null / no-op) instead of racing its shutdown.
     */
    public static <T> void deleteMantleData(NativeWorld level, int x, int y, int z, Class<T> type) {
        Engine engine = getEngine(level);
        if (engine == null) {
            return;
        }
     ...
     * with {@link #setMantleData(NativeWorld, int, int, int, Object)} and expects to read back later must be
