...
@NoArgsConstructor
@Desc("Three-dimensional rock shelves retained above the approved riverbank cut.")
@Data
public class IrisRiverBank3DConfig {
    @Desc("Retain connected volumetric shelves in eroded riverbanks.")
    private boolean enabled = true;

    @Desc("Seed offset for the riverbank density field.")
    private long seed = 0;

    @MinNumber(0)
    @MaxNumber(32)
    @Desc("Maximum shelf height above the approved bank surface. Does not deepen the river cut.")
    private double amplitude = 8;

    @MinNumber(4)
    @MaxNumber(4096)
    @Desc("Horizontal density feature size in blocks at the style's default zoom.")
    private double horizontalScale = 48;

    @MinNumber(4)
    @MaxNumber(256)
    @Desc("Vertical density feature size in blocks. Short scales form separate rock shelves.")
    private double verticalScale = 8;

    @MinNumber(1)
    @MaxNumber(16)
    @Desc("Maximum horizontal distance in blocks from an overhang to a solid supporting bank.")
    private int maximumOverhang = 8;

    @Desc("Signed XYZ density controlling retained bank rock. Supports generator styles and fractures.")
