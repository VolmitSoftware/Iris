...
@NoArgsConstructor
@Description("Represents a biome in iris. Biomes are placed inside of regions and hold objects.\nA biome consists of layers (block palletes), decorations, objects & generators.")
@Data
...
public class IrisBiome extends IrisRegistrant implements Rarity {
    ...
    @Description("If the biome type custom is defined, specify this")
    private KList<IrisBiomeCustom> customDerivitives;
    @Description("Spawn Entities in this area over time. Iris will continually replenish these mobs just like vanilla does.")
    ...
    @Description("Define custom block drops for this biome")
    ...
    @Description("The max layers to iterate below the surface for locked layer biomes (mesa).")
    ...
    @Description("Biome-level river placement, routing, profile, biome, and geometry policy. Omit to inherit region and dimension settings.")
    private IrisRiverPolicy riverPolicy = null;
    @MinNumber(1)
    @MaxNumber(512)
    @Description("The rarity of this biome (integer)")
    private int rarity = 1;
    @Description("A color for visualizing this biome with a color. I.e. #F13AF5. This will show up on the map.")
    ...
    @ArrayType(min = 1, type = String.class)
    @Description("List any biome names (file names without.json) here as children. Portions of this biome can sometimes morph into their children. Iris supports cyclic relationships such as A > B > A > B. Iris will stop checking 4 biomes down the tree.")
    private KList<String> children = new KList<>();
    @RegistryListResource(IrisBiome.class)
    @Description("Registers the referenced biome with this pack's reachable biome set. It is NOT applied as a substitute under carvings; cave biomes come from the region's caveBiomes list.")
    private String carvingBiome = "";
    @MinNumber(0)
    @MaxNumber(256)
    @Description("Minimum depth below terrain surface required before this cave biome can be selected.")
    ...
    @ArrayType(type = IrisBiomePaletteLayer.class)
    @Description("Layers of materials filling the water column of sea biomes, indexed downward from the water surface. Anything below the last layer is filled with the dimension fluid palette, not stone.")
    ...
    @Description("Procedural objects (such as trees) iris generates from scratch and places in this biome, instead of loading them from iob files")
    ...
    @Description("Optional volumetric surface terrain profile. Shapes projecting ledges, undercuts, arches, and fissures around this biome's height generators.")
    ...
    @Description("Optional exact host block ids that replace each ore deposit's surfaceReplaceableBlocks on exterior terrain inside this biome. Omit to inherit the deposit setting; an empty list forbids surface ore.")
    private KList<String> surfaceOreReplaceableBlocks = null;
    @MinNumber(0)
    @MaxNumber(1)
    @Description("Multiplier applied to the frequency of every ore deposit whose center is inside this biome. A value of 0.4 keeps 40% of ore veins while leaving non-ore deposits unchanged.")
    private double oreDepositFrequencyMultiplier = 1D;
    @MinNumber(0.01)
    @MaxNumber(16)
    @Description("Multiplier applied to the block count of every ore deposit whose center is inside this biome. Non-ore deposits are unchanged.")
    private double oreDepositSizeMultiplier = 1D;
    @ArrayType(min = 1, type = IrisDepositVariant.class)
    @Description("Deposit ore remap rules scoped to this biome. Each entry declares a vertical band and a source->replacement block id map. Applied before regional and dimension rules; first matching biome rule wins.")
