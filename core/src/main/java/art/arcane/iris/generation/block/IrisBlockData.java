...
public class IrisBlockData extends IrisRegistrant {
    ...
    @Required
    @Description("The block to use")
    private String block = "air";
    @Description("Debug this block by printing it to the console when it's used. Must have debug turned on in settings.")
    private boolean debug = false;
    @MinNumber(1)
    @MaxNumber(1000)
    @Description("The weight is used when this block data is inside of a list of blockdata. A weight of two is just as if you placed two of the same block data values in the same list making it more common when randomly picked.")
    private int weight = 1;
    @Description("If the block cannot be created on this version, Iris will attempt to use this backup block data instead.")
    private IrisBlockData backup = null;
    @RegistryMapBlockState("block")
    @Description("Optional properties for this block data such as 'waterlogged': true")
    private KMap<String, Object> data = new KMap<>();
    @Description("Optional tile data for this block data")
