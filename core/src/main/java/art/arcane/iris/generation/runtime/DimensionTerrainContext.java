...
public final class DimensionTerrainContext implements DataProvider {
    ...
    private static DimensionTerrainContext createCrossReferencing(
        ...
        ProceduralStream<IrisRegion> regionStream = focusRegion == null
                ? GenerationStreams.cache2D(proceduralRegionStream
                        .convertAware2D((region, x, z) -> mappedRegion(imageMapRuntime, region, x, z)), cachePrefix + "ImageMappedRegionStream", engine, cacheSize)
        ...
        ProceduralStream<IrisBiome> baseBiomeStream = focusBiome == null
                ? GenerationStreams.cache2D(proceduralBaseBiomeStream
                        .convertAware2D((biome, x, z) -> mappedBiome(imageMapRuntime, biome, x, z)), cachePrefix + "ImageMappedBaseBiomeStream", engine, cacheSize)
        ...
        ProceduralStream<Double> heightStream = GenerationStreams.cache2DDouble(ProceduralStream.of((x, z) -> {
        ...
        ProceduralStream<IrisBiome> finalBiomeStream = focusBiome == null
                ...
                }), cachePrefix + "ImageMappedFinalBiomeStream", engine, cacheSize)
                : GenerationStreams.cache2D(ProceduralStream.of(
                        (x, z) -> focusBiome,
                        Interpolated.of(value -> 0D, value -> focusBiome)
                ), cachePrefix + "FinalBiomeStreamFocus", engine, cacheSize);
