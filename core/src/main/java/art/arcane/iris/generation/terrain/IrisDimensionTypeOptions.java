...
import static art.arcane.iris.generation.terrain.IrisDimensionTypeOptions.TriState.DEFAULT;
...
@Description("Optional options for the dimension type")
public class IrisDimensionTypeOptions {
    @MinNumber(0.00001)
    @MaxNumber(30000000)
    @Description("The multiplier applied to coordinates when leaving the dimension. Value between 0.00001 and 30000000.0 (both inclusive).")
    private double coordinateScale = -1;
    @MinNumber(0)
    @MaxNumber(1)
    @Description("How much light the dimension has. When set to 0, it completely follows the light level; when set to 1, there is no ambient lighting.")
    private float ambientLight = -1;
    @Nullable
    @MinNumber(0)
    @MaxNumber(Long.MAX_VALUE)
    @Description("If this is set to an int, the time of the day is the specified value. To ensure a normal time cycle, set it to null.")
    private Long fixedTime = -1L;
    @Nullable
    @MinNumber(-2032)
    @MaxNumber(2031)
    @Description("Optional value between -2032 and 2031. If set, determines the lower edge of the clouds. If set to null, clouds are disabled in the dimension.")
    private Integer cloudHeight = -1;
    @MinNumber(0)
    @MaxNumber(15)
    @Description("Value between 0 and 15 (both inclusive). Maximum block light required when the monster spawns.")
    ...
    @NonNull
    @Description("When false, compasses spin randomly, and using a bed to set the respawn point or sleep, is disabled. When true, nether portals can spawn zombified piglins, and creaking hearts can spawn creakings.")
    private TriState natural = DEFAULT;
    @NonNull
    @Description("When false, Piglins and hoglins shake and transform to zombified entities.")
    private TriState piglinSafe = DEFAULT;
    @NonNull
    @Description("When false, the respawn anchor blows up when trying to set spawn point.")
    ...
    @NonNull
    @Description("Whether the dimension has skylight or not.")
    private TriState skylight = DEFAULT;
    @NonNull
    @Description("Whether the dimension has a bedrock ceiling. Note that this is only a logical ceiling. It is unrelated with whether the dimension really has a block ceiling.")
    ...
    @Description("Allows reusing the behavior of the base dimension")
    public enum TriState {
        @Description("Follow the behavior of the base dimension")
        DEFAULT,
        @Description("True")
        TRUE,
        @Description("False")
