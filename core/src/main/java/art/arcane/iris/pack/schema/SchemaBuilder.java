...
public class SchemaBuilder {
    ...
    String getType(Class<?> c) {
        ...
        if (!c.isAnnotationPresent(Description.class) && c.getCanonicalName().startsWith("art.arcane.iris.")) {
            warnings.addIfMissing("Unsupported Type: " + c.getCanonicalName() + " Did you forget @Description?");
        }
    ...
    String getFieldDescription(Field r) {
        ...
        if (r.isAnnotationPresent(Description.class)) {
            return r.getDeclaredAnnotation(Description.class).value();
        }

        warnings.addIfMissing("Missing @Description on field " + r.getName() + " (" + r.getType() + ") in " + r.getDeclaringClass().getCanonicalName());
    ...
    String getDescription(Class<?> r) {
        ...
        if (!r.isPrimitive() && !r.equals(KList.class) && !r.equals(KMap.class) && r.getCanonicalName().startsWith("art.arcane.")) {
            warnings.addIfMissing("Missing @Description on " + r.getSimpleName() + " in " + (r.getDeclaringClass() != null ? r.getDeclaringClass().getCanonicalName() : " NOSRC"));
        }
        return "";
    }
}
