package art.arcane.iris.platform.registry;
...
import static art.arcane.iris.platform.registry.KeyedRegistry.wrapped;
...
public class RegistryUtil {
    ...
    public static <T> KeyedRegistry<T> lookup(@NonNull Class<T> typeClass) {
        return (KeyedRegistry<T>) CACHE.computeIfAbsent(typeClass, $ -> {
            KList<KeyedRegistry<Object>> registries = new KList<KeyedRegistry<Object>>();
            if (Keyed.class.isAssignableFrom(typeClass)) {
                KeyedRegistry<? extends Keyed> bukkit = wrapped(getRegistry(typeClass.asSubclass(Keyed.class)));
                if (bukkit == null) {
    ...
    private static KeyedRegistry<Object> getKeyedValues(@NonNull Class<?> typeClass) {
        return wrapped(Arrays.stream(typeClass.getDeclaredFields())
                ...
                .map(field -> {
                    try {
                        Keyed value = (Keyed) field.get(null);
