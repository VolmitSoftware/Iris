...
@Description("One object placed at a fixed world position, with transformations and edits applied to a copy")
public class IrisStaticObject {
    ...
    @Description("Object load key under objects/, without the .iob extension")
    private String object;
    @Required
    @Description("Absolute world X, Y and Z for the object's saved origin. Rotation and scaling keep this origin fixed.")
    private IrisPosition position;
    @Description("Fixed rotation around the saved object origin, in degrees. All axes default to zero.")
    private IrisStaticObjectRotation rotation = new IrisStaticObjectRotation();
    @MinNumber(MINIMUM_SCALE)
    @MaxNumber(MAXIMUM_SCALE)
    @Description("Explicit fixed size multiplier. When omitted, the dimension's allObjectScaleFactor applies. An explicit 1 preserves the saved size.")
    private Double scale;
    @Description("Interpolation used when enlarging the object. NONE preserves blocky shapes.")
    ...
    @Description("Material and block-state replacements applied before rotation, without changing the source object")
    private KList<IrisObjectReplace> edit = new KList<>();
    @Description("Replace blocks inside the object's transformed bounding box with air before placing its blocks")
    private boolean bore = false;
    @Description("Fill enclosed rooms and pockets with air before placement, preserving their empty interiors")
    private boolean smartBore = false;
    @Description("Fixed seed for block replacement palettes and probabilities, independent of the world seed and chunk generation order.")
