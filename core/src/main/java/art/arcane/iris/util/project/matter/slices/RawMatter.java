...
import art.arcane.iris.util.hunk.storage.PaletteOrHunk;
...
public abstract class RawMatter<T> extends PaletteOrHunk<T> implements MatterSlice<T> {
    ...
    public RawMatter(int width, int height, int depth, Class<T> type) {
        super(width, height, depth, true, () -> Hunk.adapt(new art.arcane.volmlib.util.hunk.storage.MappedHunk<>(width, height, depth)));
        writers = new KMap<>();
        readers = new KMap<>();
        this.type = type;
    }
