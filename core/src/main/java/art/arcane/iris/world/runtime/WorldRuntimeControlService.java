...
public final class WorldRuntimeControlService {
    ...
    public boolean applyStudioWorldRules(World world) {
        ...
        IrisServices.get(art.arcane.iris.integration.MultiverseCoreLink.class).removeIfPresent(world);
    ...
    public void prepareGenerator(World world) {
        ...
        try {
            art.arcane.iris.platform.generation.PlatformChunkGenerator provider = art.arcane.iris.world.IrisToolbelt.access(world);
            if (provider == null) {
                return;
            }

            art.arcane.iris.generation.runtime.Engine engine = provider.getEngine();
            if (engine == null) {
                return;
            }
    ...
    public static Location resolveEntryAnchor(World world, PlatformChunkGenerator provider) {
        if (world == null) {
            return null;
        }
