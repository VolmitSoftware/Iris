...
public class StructureCarvingFootprintTest {
    ...
    public void preservesIrregularFootprintInsteadOfFillingItsBoundingBox() {
        ...
        StructureCarvingFootprint footprint = StructurePieceColumns.from(pieces(piece), 0);
    ...
    public void reportsExactDistanceAndNearestColumnHeights() {
        ...
        StructureCarvingFootprint footprint = StructurePieceColumns.from(pieces(piece), 2);
    ...
    public void matchesBruteForceSquaredDistancesAcrossSparseColumns() {
        ...
        StructureCarvingFootprint footprint = StructurePieceColumns.from(pieces(piece), 3);
    ...
    public void returnsNullForEmptyInvalidOrOversizedInputs() {
        assertNull(StructurePieceColumns.from(null, 0));
        assertNull(StructurePieceColumns.from(new KList<>(), 0));
        ...
        assertNull(StructurePieceColumns.from(invalidPieces, 0));

        PlacedStructurePiece piece = piece(0, 0, 0, IrisObjectRotation.of(0, 0, 0), block(0, 0, 0));
        assertNull(StructurePieceColumns.from(pieces(piece), 2, 24));
        PlacedStructurePiece overflowing = piece(
                Integer.MAX_VALUE, 0, 0, IrisObjectRotation.of(0, 0, 0), block(1, 0, 0));
        assertNull(StructurePieceColumns.from(pieces(overflowing), 0));

        try {
            StructurePieceColumns.from(pieces(piece), -1);
    ...
    public void includesTheEntirePaddingBoundary() {
        ...
        StructureCarvingFootprint footprint = StructurePieceColumns.from(pieces(piece), 2, 25);
